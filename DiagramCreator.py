...
class diagram_window(tk.Frame):
    def __init__(self, master):
        ...
        # in general this will be opend by main.py and is a tab
        self.isTab = False
        try:
    ...
    def top_widgets(self):
        ...
        # i personally do not want a tittle if this window is opend in a tab
        # may delete this in a future version
    ...
    def bottom_widgets(self):
        ...
        # if DiagramCreator is started by Main, do not create quitButton
        if __name__ == "__main__":
        #if not self.isTab:
    ...
    def create_Fig(self, figsize=4, DONESTATUS="Done", PiechartBottomLeftColumnTitle="Prio"):
        ...
        # Date fig
        self.fig = Figure(figsize=(figsize * 2 * self.DIAGRAMSCALE, figsize * self.DIAGRAMSCALE), facecolor="white")
        ...
        # Status Pie Chart
        self.fig2 = Figure(figsize=(figsize * self.DIAGRAMSCALE, figsize * self.DIAGRAMSCALE), facecolor="white")
        ...
        self.fig3 = Figure(figsize=(figsize * self.DIAGRAMSCALE, figsize * self.DIAGRAMSCALE), facecolor="white")
        ...
        canvas.draw()
        canvas.get_tk_widget().grid(column=0, row=0, columnspan=2, padx=2, pady=2)
        canvas2 = FigureCanvasTkAgg(self.fig2, master=self.midFrame)
        canvas2.draw()
        canvas2.get_tk_widget().grid(column=0, row=1, padx=2, pady=2)
        canvas3 = FigureCanvasTkAgg(self.fig3, master=self.midFrame)
        canvas3.draw()
        canvas3.get_tk_widget().grid(column=1, row=1, padx=2, pady=2)
